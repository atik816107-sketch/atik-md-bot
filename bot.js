...
const checkUserJoinedChannels = async (userId) => {
  // Always return true to skip join check
  return true;
};

// ========== SEND CHANNELS REQUIRED MESSAGE (DISABLED) ==========
const sendChannelsRequiredMessage = async (chatId) => {
  // This function is no longer used but kept to avoid reference errors
  return null;
};
...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data === 'check_join') {
    ...
    await bot.answerCallbackQuery(callbackQuery.id, { 
      text: '✅ Join check disabled. You can use the bot.', 
      show_alert: true
    });
    await bot.sendMessage(chatId, '✅ *Join check is disabled!*\n\nNow send /pair to start pairing.', { parse_mode: 'Markdown' });
    return;
  }
});
